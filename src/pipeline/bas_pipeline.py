...
from src.utils.stats import masked_adain, adain
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        blend_latents_timesteps: Optional[List[int]] = None,
        bg_latents_list: Optional[List] = None,
        fg_latents_list: Optional[List] = None,
        ...
        Args:
            ...
            return_latents (bool, *optional*, defaults to False):
                Whether to return the latent representations of the generated image.
            latents_mask (torch.FloatTensor, *optional*):
                Mask for Latent Blending. $\\mathcal{M}_\\text{BG}$ in the paper.
            blend_latents_timesteps (List[int], *optional*):
                A list of timesteps at which we do latent blending.
            bg_latents_list (List, *optional*):
                List of latents for all timesteps for the previous generation stage.
            fg_latents_list (List, *optional*):
                 List of latents for all timesteps for the inverted warped image..
            use_adain (bool, *optional*, defaults to False):
                Whether to use Adaptive Instance Normalization (AdaIN) after latent blending
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ################# START Build-A-Scene #################
                if (  # Used when adding a new object to preserve the background
                    bg_latents_list
                    and fg_latents_list is None
                    and latents.shape[0] > 1
                    and blend_latents_timesteps
                    and i in blend_latents_timesteps
                ):
                    # print("Mapping latents at step", i)
                    latents[0] = bg_latents_list[i][-1]  # Replace with x_T from the previous stage
                    latents[1] = latents_mask * latents[0] + (1 - latents_mask) * latents[1]
                    if use_adain:
                        # print("Using AdaIN")
                        # latents[1] = masked_adain(latents[1], latents[0], 1 - latents_mask, latents_mask)
                        latents[-1] = adain(latents[1], latents[0])

                elif (  # Used when calling "Consistent 3D generation" to maintain the background and the warped object
                    fg_latents_list
                    and bg_latents_list
                    and latents.shape[0] > 1
                    and blend_latents_timesteps
                    ...
                    # print("Mapping FG latents for image 1 at step", i)
                    latents[1] = latents_mask * latents[1] + (1 - latents_mask) * fg_latents_list[-i][0]
                    if use_adain:
                        # print("Using AdaIN")
                        # latents[1] = masked_adain(latents[1], latents[0], 1 - latents_mask, latents_mask)
                        latents[-1] = adain(latents[1], latents[0])

                # Store all latents to be used in following stages
                if return_latents:
        ...
        if return_latents:
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), latents_list
        else:
    ...
    def image2latent(self, image, device):
        ...
        if type(image) is Image:
            ...
            image = image.permute(2, 0, 1).unsqueeze(0).to(DEVICE).to(self.unet.dtype)
    ...
    def invert_depth(
        ...
        **kwds,
        ...
        for i, t in enumerate(tqdm(reversed(self.scheduler.timesteps), desc="DDIM Inversion")):
            ...
            # Control Net
            control_model_input = model_inputs
