...
def regiter_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def ca_forward(self, place_in_unet):
        def forward(x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None):
            ...
            # Blending features after self-attention to improve conssiency
            hidden_states = out
            res = int(np.sqrt(hidden_states.shape[1]))
            if editor.blend_sattn and not is_cross and res < 32 and x.shape[0] > 2 and editor.mask is not None:
                # print(hidden_states.shape)
                mask = editor.mask.unsqueeze(0).unsqueeze(0)
                mask_t_res = F.interpolate(mask, (res, res), mode="bilinear").to(hidden_states.device).reshape(1, -1, 1)
                hidden_states[1] = (1 - mask_t_res) * hidden_states[0] + mask_t_res * hidden_states[1]
                hidden_states[3] = (1 - mask_t_res) * hidden_states[2] + mask_t_res * hidden_states[3]
                out = hidden_states
            return out
...
def regiter_attention_editor_ldm(model, editor: AttentionBase):
    ...
    def register_editor(net, count, place_in_unet):
        for name, subnet in net.named_children():
            if "CrossAttention" in net.__class__.__name__:  # spatial Transformer layer
                net.forward = ca_forward(net, place_in_unet)
                return count + 1
            elif hasattr(net, "children"):
                count = register_editor(subnet, count, place_in_unet)
